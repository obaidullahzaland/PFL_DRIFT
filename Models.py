...
class client_model(nn.Module):
    def __init__(self, name, num_classes=10, img_size=(32,32)):
        ...
        if 'LeNet' in self.name:
            ...
            self.fc1 = nn.Linear(in_feat, 384)
    ...
    def forward(self, x):
        if 'LeNet' in self.name:
            ...
            x = x.flatten(start_dim=1) # changed as view only works with images 32x32
            # x = x.view(-1, 64 * 5 * 5)
            x = F.relu(self.fc1(x))
            x = F.relu(self.fc2(x))
            x = self.fc3(x)
